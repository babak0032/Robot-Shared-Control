...
#define NR 1      /* number of robots */
#define L 10        /* number of positions: 0: center, 1: wall, 2: corner*/
#define TT0 40000000L
#define TM 20000L /* Seems like the amount of run time which will terminate the graphics */
#define TRUE    1
#define FALSE   0
...
struct Robot {
    ...
    double gbx,gby;
    double gf,pf;
    int neighbours[NEIGHBOUR_NUMBER];
};
...
void InitRobot(int r,struct Robot *robot) {
  ...
  // At the begining: let all the neighbours be himself:
    int i;
    for ( i = 0; i < NEIGHBOUR_NUMBER; i++ ) {
      robot->neighbours[i] = r; 
    }
}

void sort(double* number, int n) {  
...
void RunRobot(int my_r,struct Robot *robot) {
  ...
  //double *distances[NR-1];
  double *distances = malloc((NR-1) * sizeof *distances);
  double indexes[NR-1];
  ...
  int i;
  for (r=0;r<NEIGHBOUR_NUMBER;r++) {
    neighbour_dist = distances[r];
    for (i=0;r<NR-1;r++) {
      if (indexes[i] == neighbour_dist) {
        robot->neighbours[r] = i;
      }
    }
  }

  double neighbour_center_X = 0.0;
  double neighbour_center_Y = 0.0;
  int neighbour;

  for (r=0;r<NEIGHBOUR_NUMBER;r++) {
    neighbour = robot->neighbours[r];
    neighbour_center_X = neighbour_center_X + robots[neighbour]->p_x;
    neighbour_center_Y = neighbour_center_Y + robots[neighbour]->p_y;
  }

  neighbour_center_X = neighbour_center_X/NEIGHBOUR_NUMBER;
  neighbour_center_Y = neighbour_center_Y/NEIGHBOUR_NUMBER;


  robot->v_x=w*robot->v_x+a1*drand48()*(robot->pbx-robot->p_x)+a2*drand48()*(robot->gbx-robot->p_x)+ac*(robot->p_x-robots[mrad]->p_x)/(radmin+0.0001) + attract*(neighbour_center_X - robot->p_x);
  robot->v_y=w*robot->v_y+a1*drand48()*(robot->pby-robot->p_y)+a2*drand48()*(robot->gby-robot->p_y)+ac*(robot->p_y-robots[mrad]->p_y)/(radmin+0.0001) + attract*(neighbour_center_Y - robot->p_y);
