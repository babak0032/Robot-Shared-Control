...
void NetDisplay() {
	...
	Color(RED);
	DrawRectangle((WINDOW_H+4)/2,(WINDOW_H+4)/2,(WINDOW_H-8)/2,(WINDOW_H-10)/2);
	Color(BLUE);
	DrawRectangle(4,4,(WINDOW_H-4)/2,(WINDOW_H-4)/2);
...
void InitRobot(int r,struct Robot *robot) {
	...
	robot->p_x=0.5+0.45*robot->p_x;
	robot->p_y=0.5+0.45*robot->p_y;
}

void RunRobot(long t,int my_r,struct Robot *robot) {
	int r,rg,oo,mrad=my_r;
	...
	for (r=0;r<NR;r++) {
		...
		if ((robots[rg]->p_x<0.5)&&(robots[rg]->p_y<0.5)&&
		((robot->p_x>0.5)||(robot->p_y>0.5))) {
	...
	if ((robot->p_x<0.5)&&(robot->p_y<0.5)) {
		robot->pbx=robot->p_x;
		robot->pby=robot->p_y;
	}
	else {
