...
void NetDisplay() {
    ...
    DrawRectangle((WINDOW_H)/3,(WINDOW_H*2)/3,4,(WINDOW_H)/3); // vertical
    DrawRectangle((WINDOW_H*2)/3,(WINDOW_H*2)/3,4,(WINDOW_H)/3); // vertical      
...
void RunRobot(int my_r,struct Robot *robot) {
  ...
  if ((fabs(robot->p_y-0.5)>0.169)&&(((robot->p_xo<0.69)&&(robot->p_x>0.64))||((robot->p_xo>0.69)&&(robot->p_x<0.64)))) {
  robot->p_x=robot->p_xo;
  } 
